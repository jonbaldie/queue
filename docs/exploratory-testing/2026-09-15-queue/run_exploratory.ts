...
async function journey2ProtocolValidationAndLimits(): Promise<void> {
    ...
    try {
        ...
        record("j2-protocol", "auth-scheme-lowercase", await request(server, "/queues", { headers: { Authorization: "bearer explore-token-20260915" } }), "200 OK");
        record("j2-protocol", "auth-scheme-uppercase", await request(server, "/queues", { headers: { Authorization: "BEARER explore-token-20260915" } }), "200 OK");
        record("j2-protocol", "auth-multiple-spaces", await request(server, "/queues", { headers: { Authorization: "Bearer    explore-token-20260915" } }), "200 OK");
        record("j2-protocol", "auth-tab-delimited", await request(server, "/queues", { headers: { Authorization: "Bearer\texplore-token-20260915" } }), "200 OK");
